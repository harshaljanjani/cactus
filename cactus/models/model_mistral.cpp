...
MistralModel::MistralModel() : Model() {}

MistralModel::MistralModel(const Config& config) : Model(config) {
    weight_nodes_.layers.resize(config.num_layers);
}

void MistralModel::post_init() {
    if (config_.sliding_window == 0 || config_.sliding_window >= config_.context_length) {
        kv_cache_.set_window_size(0, 0);
    }
}

void MistralModel::load_weights_to_graph(CactusGraph* gb) {
...
size_t MistralModel::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    // https://github.com/keras-team/keras-hub/blob/master/keras_hub/src/models/mistral/mistral_transformer_decoder.py#L215-L224
    size_t attn_output_4d;
    if (config_.sliding_window > 0 && config_.sliding_window < config_.context_length) {
        attn_output_4d = gb->attention(q_proj_4d, final_k, final_v, attention_scale_, position_offset, config_.sliding_window, backend);
    } else {
        attn_output_4d = gb->attention(q_proj_4d, final_k, final_v, attention_scale_, position_offset, backend);
    }
...
size_t MistralModel::build_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx,
                                ComputeBackend backend) const {
    const auto& layer = weight_nodes_.layers[layer_idx];

    // gate_proj(x) * silu(up_proj(x)) -> down_proj
    // https://github.com/huggingface/transformers/blob/v4.57.3/src/transformers/models/mistral/modeling_mistral.py#L46-L48
    // https://github.com/keras-team/keras-hub/blob/master/keras_hub/src/models/mistral/mistral_transformer_decoder.py#L162-L177
...
size_t MistralModel::build_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx,
                                              ComputeBackend backend, bool use_cache, size_t position_offset) {
    const auto& layer = weight_nodes_.layers[layer_idx];

    // https://github.com/huggingface/transformers/blob/v4.57.3/src/transformers/models/mistral/modeling_mistral.py#L228-L248
    // https://github.com/keras-team/keras-hub/blob/master/keras_hub/src/models/mistral/mistral_transformer_decoder.py#L141-L179
...
size_t MistralModel::forward(const std::vector<uint32_t>& tokens, bool use_cache) {
    ...
    // https://github.com/huggingface/transformers/blob/v4.57.3/src/transformers/models/mistral/modeling_mistral.py#L379
